...
public interface LotRepository extends JpaRepository<Lot, Long> {
    ...
    @Query(value = "SELECT l.id FROM Lot l WHERE l.endDate < :now AND l.isActive = TRUE",
            countQuery = "SELECT count(l.id) FROM Lot l WHERE l.endDate < :now AND l.isActive = TRUE")
    ...
    List<Lot> getLotsWithBidsByIds(List<Long> ids);
}
